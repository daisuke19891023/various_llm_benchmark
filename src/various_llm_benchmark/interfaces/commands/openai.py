...
REASONING_EFFORT_OPTION: ReasoningEffort | None = typer.Option(
    None,
    help="OpenAI Responses APIのreasoning.effortを指定します (none/low/medium/high)。",
    show_default=False,
)
VERBOSITY_OPTION: Verbosity | None = typer.Option(
    None,
    help="OpenAI Responses APIのtext.verbosityを指定します (low/medium/high)。",
    show_default=False,
)
...
def openai_complete(
    ...
    reasoning_effort: ReasoningEffort | None = REASONING_EFFORT_OPTION,
    verbosity: Verbosity | None = VERBOSITY_OPTION,
) -> None:
    """Generate a single-turn response with OpenAI."""
    with console.status("OpenAIで応答生成中...", spinner="dots"):
...
def openai_chat(
    ...
    with console.status("OpenAIで履歴付き応答を生成中...", spinner="dots"):
...
def openai_vision(
    ...
    with console.status("OpenAIで画像を解析中...", spinner="dots"):
